...
- (1) `非 upstream 补丁添加 ck: 前缀`: 根据 2.3.2-(1) 的规则，我们希望尽量少引入非 Upstream 的补丁，但是如果真的有这样的补丁，我们希望在补丁标题上就可以鉴别出来，所以应该在补丁标题上添加 `ck:` 的前缀，举例如下：

   ```bash
   ck: hotfix: Add Kernel hotfix enhancement

   <snip the contents>
   ```

- (2) `大补丁系列鼓励使用关键字`: 有时候你会提交一个大补丁系列，包含了20多个甚至更多补丁。这种情况下我们推荐在每个补丁的标题栏中添加一个关键字前缀。例如在一个补丁系列中，我们要引入一个新的硬件平台，单个补丁可能显示如下：
