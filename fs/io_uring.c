...
static void io_sq_thread_drop_mm_files(struct io_ring_ctx *ctx)
{
...
static void io_async_task_func(struct callback_head *cb)
{
	...
	if (!canceled) {
		__set_current_state(TASK_RUNNING);
		if (io_sq_thread_acquire_mm_files(ctx, req)) {
...
static struct io_wq_work *io_wq_submit_work(struct io_wq_work *work)
{
	...
	if (ret) {
		 ...
		 * so here for polled io, we also get uring_lock to complete it.
		 */
		if (lock_ctx)
...
static int io_sq_thread(void *data)
{
	...
	while (!kthread_should_park()) {
		...
		if (!to_submit || ret == -EBUSY || need_resched()) {
			 ...
			 * may sleep.
			 */
			io_sq_thread_drop_mm_files(ctx);
...
static void io_sqd_update_thread_idle(struct sq_thread_percpu *t)
{
	...
	list_for_each_entry(ctx, &t->ctx_list, sqd_list) {
		...
		if (idle_mode_us && !t->idle_mode_us)
			t->idle_mode_us = true;
...
static void io_sq_cgrp_migr_work(struct work_struct *work)
{
	...
	t = per_cpu_ptr(percpu_threads, info->cpu);
	...
	pid = task_pid_nr(t->sqo_thread);
	snprintf(pid_str, sizeof(pid_str), "%d", pid);
	io_sq_thread_park(t);
	t->sqo_thread->flags &= ~PF_NO_SETAFFINITY;
...
static int io_sq_attach_cpu_cgroup(int cpu)
{
	struct cgroup_subsys_state *css_owner, *css_sq;
	char *buf = NULL;
	int ret = 0, len, size = 0;
	struct cgrp_migr_info info;
	struct sq_thread_percpu *t = per_cpu_ptr(percpu_threads, cpu);
	...
	css_sq = task_get_css(t->sqo_thread, cpuacct_cgrp_id);
	mutex_unlock(&t->lock);
...
static int io_sq_thread_percpu(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (unlikely(!list_empty(&t->ctx_new_list))) {
			io_sqd_init_new(t);
			timeout = io_current_time(t->idle_mode_us) + t->sq_thread_idle;
		}
		...
		if (sqt_spin || !io_time_after(t->idle_mode_us, timeout)) {
			if (current->task_works)
				task_work_run();
			cond_resched();
			/*
