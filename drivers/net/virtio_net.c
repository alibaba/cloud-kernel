...
static void virt_xsk_complete(struct send_queue *sq, u32 num, bool xsk_wakeup)
{
	...
	if (!umem) {
		if (sq->xsk.hdr_pro - sq->xsk.hdr_con == sq->xsk.hdr_n) {
			...
			xsk_umem_unpin_pages(sq->xsk.pgs, sq->xsk.npgs);
			kfree(hdr);
		}
		rcu_read_unlock();
		return;
	...
	xsk_umem_complete_tx(sq->xsk.umem, num);
...
static int virtnet_poll_tx(struct napi_struct *napi, int budget)
{
	...
	umem = rcu_dereference(sq->xsk.umem);
	if (umem)
		work = virtnet_xsk_run(sq, umem, budget);
	else
		free_old_xmit_skbs(sq, true);
				   ...
				   u16 qid)
{
	...
	ret = -EBUSY;
	if (rcu_dereference(sq->xsk.umem))
		goto end;
...
static int virtnet_xsk_pool_disable(struct net_device *dev, u16 qid)
{
	...
	if (sq->xsk.hdr_pro - sq->xsk.hdr_con == sq->xsk.hdr_n) {
		...
		sq->xsk.pgs = xsk_umem_pgs_delay_unpin(umem, &sq->xsk.npgs);
	}
...
static int virtnet_xsk_poll_query(struct net_device *dev,
				  struct netdev_bpf *xdp)
{
	struct virtnet_info *vi = netdev_priv(dev);
	struct send_queue *sq;
	u16 qid = xdp->xsk.queue_id;
...
static int virtnet_xdp(struct net_device *dev, struct netdev_bpf *xdp)
{
	...
	case XDP_SETUP_XSK_UMEM:
		if (xdp->xsk.umem)
			return virtnet_xsk_pool_enable(dev, xdp->xsk.umem,
						       xdp->xsk.queue_id);
		else
			return virtnet_xsk_pool_disable(dev, xdp->xsk.queue_id);
	case XDP_QUERY_XSK_UMEM:
		return virtnet_xsk_poll_query(dev, xdp);
	default:
		return -EINVAL;
	}
}
...
static int virtnet_xsk_xmit(struct send_queue *sq, struct xdp_desc *desc)
{
	...
	data = xdp_umem_get_data(sq->xsk.umem, desc->addr);
	page = xdp_umem_get_page(sq->xsk.umem, desc->addr);
	...
	sg_set_page(sq->sg + 1, page, desc->len, offset_in_page(data));
...
static int virtnet_xsk_xmit_zc(struct send_queue *sq,
			       struct xdp_umem *umem, unsigned int budget)
{
...
static int virtnet_xsk_wakeup(struct net_device *dev, u32 qid)
{
	struct virtnet_info *vi = netdev_priv(dev);
	struct send_queue *sq;
	struct xdp_umem *umem;
	...
	work = virtnet_xsk_run(sq, umem, xsk_budget);
...
static const struct net_device_ops virtnet_netdev = {
	...
	.ndo_xsk_async_xmit	= virtnet_xsk_wakeup,
