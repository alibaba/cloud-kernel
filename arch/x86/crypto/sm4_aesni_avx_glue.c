			...
			unsigned int key_len)
{
	...
	return crypto_sm4_expand_key(ctx, key, key_len);
}
...
int sm4_cbc_encrypt(struct skcipher_request *req)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		while (nbytes >= SM4_BLOCK_SIZE) {
			...
			sm4_do_crypt(ctx->rkey_enc, (u32 *)dst, (u32 *)dst);
			iv = dst;
			src += SM4_BLOCK_SIZE;
			dst += SM4_BLOCK_SIZE;
...
int sm4_cfb_encrypt(struct skcipher_request *req)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		while (nbytes >= SM4_BLOCK_SIZE) {
			sm4_do_crypt(ctx->rkey_enc, (u32 *)keystream, (u32 *)iv);
		...
		if (walk.nbytes == walk.total && nbytes > 0) {
			sm4_do_crypt(ctx->rkey_enc, (u32 *)keystream, (u32 *)walk.iv);
							...
							unsigned int bsize, sm4_crypt_func func)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		if (walk.nbytes == walk.total && nbytes > 0) {
			...
			sm4_do_crypt(ctx->rkey_enc, (u32 *)keystream, (u32 *)keystream);
